...
def run_pipeline(cfg: PipelineConfig, pulse_recording, dry_wav=None):
    out_dir = Path(cfg.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    gen, fit = invert_acoustic_params(cfg, pulse_recording)
    rir, meta = generate_single_rir(gen, seed=cfg.seed + 1, use_drr_c50=cfg.use_drr_c50)
    ...
    peak = float(np.max(np.abs(wet))) if wet.size > 0 else 0.0
    if peak > 0.99:
        wet = wet / peak * 0.99
